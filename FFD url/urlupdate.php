<?php
    ...
    if ($result=mysqli_query($con,$sql))
    {
        ...
        $code .= tab() . '$sql = "update '. getTableNameFromPrimaryKey($primaryKey) . ' set ' . $dbColumnName[1] . ' = ' . "'" . '$' . makeFirstLetterLowerCase($dbColumnName[1]) . "'";
        for($j=2;$j<sizeof($dbColumnName);$j++)
        {
            $code .= ', ' . $dbColumnName[$j] . ' = ' . "'" . '$' . makeFirstLetterLowerCase($dbColumnName[$j]) . "'";
        }
        $code .= ' where ' . $primaryKey . ' = ' . "'" . '$' . makeFirstLetterLowerCase($primaryKey) . "'" . '";<br>';

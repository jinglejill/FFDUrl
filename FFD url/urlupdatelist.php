<?php
    ...
    if ($result=mysqli_query($con,$sql))
    {
        ...
        $code .= tab() . tab() . tab() . '$sql = "update '. getTableNameFromPrimaryKey($primaryKey) . ' set ' . $dbColumnName[1] . ' = ' . "'" . '$' . makeFirstLetterLowerCase($dbColumnName[1]) . '[$i]' . "'";
        for($j=2;$j<sizeof($dbColumnName);$j++)
        {
            $code .= ', ' . $dbColumnName[$j] . ' = ' . "'" . '$' . makeFirstLetterLowerCase($dbColumnName[$j]) . '[$i]' .  "'";
        }
        $code .= ' where ' . $primaryKey . ' = ' . "'" . '$' . makeFirstLetterLowerCase($primaryKey) . '[$i]' . "'" . '";<br>';
        ...
        $code .= tab() . tab() . '$sql = "select *, 1 IdInserted from ' . getTableNameFromPrimaryKey($primaryKey) . ' where ' . $primaryKey . ' in ' . "('" . '$' . makeFirstLetterLowerCase($primaryKey) . '[0]' . "'" . '";<br>';

<?php    
    ...
    if($countRewardPoint > 0)
    {
        ...
        {
            //query statement
            $sql = "update RewardPoint set MemberID = '$memberID[$i]', ReceiptID = '$receiptID[$i]', Point = '$point[$i]', Status = '$status[$i]', ModifiedUser = '$modifiedUser[$i]', ModifiedDate = '$modifiedDate[$i]' where RewardPointID = '$rewardPointID[$i]'";
            $ret = doQueryTask($sql);
            if($ret != "")
            {
                mysqli_rollback($con);

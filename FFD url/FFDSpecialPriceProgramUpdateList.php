<?php
    ...
    if($countSpecialPriceProgram > 0)
    {
        ...
        {
            //query statement
            $sql = "update SpecialPriceProgram set MenuID = '$menuID[$i]', StartDate = '$startDate[$i]', EndDate = '$endDate[$i]', SpecialPrice = '$specialPrice[$i]', ModifiedUser = '$modifiedUser[$i]', ModifiedDate = '$modifiedDate[$i]' where SpecialPriceProgramID = '$specialPriceProgramID[$i]'";
            $ret = doQueryTask($sql);
            if($ret != "")
            {
                mysqli_rollback($con);

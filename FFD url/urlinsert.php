<?php
    ...
    if ($result=mysqli_query($con,$sql))
    {
        ...
        $code .= tab() . '$sql = "INSERT INTO ' . getTableNameFromPrimaryKey($primaryKey) . '(' .$dbColumnName[1];
        ...
        $code .= '//insert ผ่าน<br>';
        $code .= '$newID = mysqli_insert_id($con);<br>';
        $code .= '<br><br><br>';
        ...
        $code .= tab() . tab() . '$sql = "select ' . "$" . makeFirstLetterLowerCase($primaryKey) . ' as ' . $primaryKey . ', 1 as ReplaceSelf, '. "'" . '$modifiedUser' . "'" . ' as ModifiedUser";<br>';
        ...
        $code .= tab() . tab() . '//select row ที่แก้ไข ขึ้นมาเก็บไว้<br>';
        $code .= tab() . tab() . '$' . makeFirstLetterLowerCase($primaryKey) . ' = $newID;<br>';
        $code .= tab() . tab() . '$sql = "select *, 1 IdInserted from ' . getTableNameFromPrimaryKey($primaryKey) . ' where ' . $primaryKey .' = ' . "'" . '$' . makeFirstLetterLowerCase($primaryKey). "'". '";<br>';
        $code .= tab() . tab() . '$selectedRow = getSelectedRow($sql);<br>';
        $code .= '<br><br><br>';
        ...
        $code .= tab() . '$sql = "select *, 1 IdInserted from ' . getTableNameFromPrimaryKey($primaryKey) . ' where ' . $primaryKey . ' = ' . "'" . '$' . makeFirstLetterLowerCase($primaryKey) . "'" . '";<br>';

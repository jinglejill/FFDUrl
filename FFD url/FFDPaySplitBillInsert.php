...
//    @[inDbOrderTakingList,inDbOrderNoteList,inDbOrderKitchenList,splitOrderTakingList,splitOrderNoteList,splitOrderKitchenList, billPrintList, rewardPointList,tableTakingList,_usingReceipt];
    ...
    if(isset($_POST["trReceiptID"]) && isset($_POST["trCustomerTableID"]) && isset($_POST["trMemberID"]) && isset($_POST["trServingPerson"]) && isset($_POST["trCustomerType"]) && isset($_POST["trOpenTableDate"]) && isset($_POST["trCashAmount"]) && isset($_POST["trCashReceive"]) && isset($_POST["trCreditCardType"]) && isset($_POST["trCreditCardNo"]) && isset($_POST["trCreditCardAmount"]) && isset($_POST["trTransferDate"]) && isset($_POST["trTransferAmount"]) && isset($_POST["trRemark"]) && isset($_POST["trDiscountType"]) && isset($_POST["trDiscountAmount"]) && isset($_POST["trDiscountReason"]) && isset($_POST["trStatus"]) && isset($_POST["trReceiptNoID"]) && isset($_POST["trReceiptNoTaxID"]) && isset($_POST["trReceiptDate"]) && isset($_POST["trMergeReceiptID"]) && isset($_POST["trModifiedUser"]) && isset($_POST["trModifiedDate"]))
    {
        $trReceiptID = $_POST["trReceiptID"];
        $trCustomerTableID = $_POST["trCustomerTableID"];
        $trMemberID = $_POST["trMemberID"];
        $trServingPerson = $_POST["trServingPerson"];
        $trCustomerType = $_POST["trCustomerType"];
        $trOpenTableDate = $_POST["trOpenTableDate"];
        $trCashAmount = $_POST["trCashAmount"];
        $trCashReceive = $_POST["trCashReceive"];
        $trCreditCardType = $_POST["trCreditCardType"];
        $trCreditCardNo = $_POST["trCreditCardNo"];
        $trCreditCardAmount = $_POST["trCreditCardAmount"];
        $trTransferDate = $_POST["trTransferDate"];
        $trTransferAmount = $_POST["trTransferAmount"];
        $trRemark = $_POST["trRemark"];
        $trDiscountType = $_POST["trDiscountType"];
        $trDiscountAmount = $_POST["trDiscountAmount"];
        $trDiscountReason = $_POST["trDiscountReason"];
        $trStatus = $_POST["trStatus"];
        $trReceiptNoID = $_POST["trReceiptNoID"];
        $trReceiptNoTaxID = $_POST["trReceiptNoTaxID"];
        $trReceiptDate = $_POST["trReceiptDate"];
        $trMergeReceiptID = $_POST["trMergeReceiptID"];
        $trModifiedUser = $_POST["trModifiedUser"];
        $trModifiedDate = $_POST["trModifiedDate"];
    }
    ...
    if($countSotOrderTaking > 0)
    {
        for($k=0; $k<$countSotOrderTaking; $k++)
        {
            ...
            //select row ที่แก้ไข ขึ้นมาเก็บไว้
            $orderTakingOldNew[$sotOrderTakingID[$k]] = $newID;
    ...
    //receipt tr
    //query statement
    $sql = "update Receipt set CustomerTableID = '$trCustomerTableID', MemberID = '$trMemberID', ServingPerson = '$trServingPerson', CustomerType = '$trCustomerType', OpenTableDate = '$trOpenTableDate', CashAmount = '$trCashAmount', CashReceive = '$trCashReceive', CreditCardType = '$trCreditCardType', CreditCardNo = '$trCreditCardNo', CreditCardAmount = '$trCreditCardAmount', TransferDate = '$trTransferDate', TransferAmount = '$trTransferAmount', Remark = '$trRemark', DiscountType = '$trDiscountType', DiscountAmount = '$trDiscountAmount', DiscountReason = '$trDiscountReason', Status = '$trStatus', ReceiptNoID = '$receiptNoID', ReceiptNoTaxID = '$trReceiptNoTaxID', ReceiptDate = '$trReceiptDate', MergeReceiptID = '$trMergeReceiptID', ModifiedUser = '$trModifiedUser', ModifiedDate = '$trModifiedDate' where ReceiptID = '$trReceiptID'";
    $ret = doQueryTask($sql);
    if($ret != "")
    {
        mysqli_rollback($con);
    ...
    //select row ที่แก้ไข ขึ้นมาเก็บไว้
    $sql = "select *, 1 IdInserted from Receipt where ReceiptID = '$trReceiptID'";
    $selectedRow = getSelectedRow($sql);

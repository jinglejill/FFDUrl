<?php
    ...
    if($countReceipt > 0)
    {
        ...
        for($k=0; $k<$countReceipt; $k++)
        {
            //query statement
            $sql = "INSERT INTO Receipt(CustomerTableID, MemberID, ServingPerson, CustomerType, OpenTableDate, CashAmount, CashReceive, CreditCardType, CreditCardNo, CreditCardAmount, TransferDate, TransferAmount, Remark, DiscountType, DiscountAmount, DiscountReason, Status, ReceiptNoID, ReceiptNoTaxID, ReceiptDate, MergeReceiptID, ModifiedUser, ModifiedDate) VALUES ('$rtCustomerTableID[$k]', '$rtMemberID[$k]', '$rtServingPerson[$k]', '$rtCustomerType[$k]', '$rtOpenTableDate[$k]', '$rtCashAmount[$k]', '$rtCashReceive[$k]', '$rtCreditCardType[$k]', '$rtCreditCardNo[$k]', '$rtCreditCardAmount[$k]', '$rtTransferDate[$k]', '$rtTransferAmount[$k]', '$rtRemark[$k]', '$rtDiscountType[$k]', '$rtDiscountAmount[$k]', '$rtDiscountReason[$k]', '$rtStatus[$k]', '$receiptNoID', '$rtReceiptNoTaxID[$k]', '$rtReceiptDate[$k]', '$rtMergeReceiptID[$k]', '$rtModifiedUser[$k]', '$rtModifiedDate[$k]')";
            $ret = doQueryTask($sql);
            if($ret != "")
            {
                mysqli_rollback($con);
            ...
            //**********sync device token ตัวเอง delete old id and insert newID
            //select row ที่แก้ไข ขึ้นมาเก็บไว้
            $sql = "select $rtReceiptID[$k] as ReceiptID, 1 as ReplaceSelf, '$rtModifiedUser[$k]' as ModifiedUser";
            $selectedRow = getSelectedRow($sql);
            ...
            //select row ที่แก้ไข ขึ้นมาเก็บไว้
            $rtReceiptID[$k] = $newID;
            $sql = "select *, 1 IdInserted from Receipt where ReceiptID = '$rtReceiptID[$k]'";
            $selectedRow = getSelectedRow($sql);

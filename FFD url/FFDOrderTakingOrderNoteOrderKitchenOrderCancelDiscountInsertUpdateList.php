<?php
    ...
    if(isset($_POST["ocdOrderCancelDiscountID"]) && isset($_POST["ocdOrderTakingID"]) && isset($_POST["ocdType"]) && isset($_POST["ocdDiscountType"]) && isset($_POST["ocdDiscountAmount"]) && isset($_POST["ocdReason"]) && isset($_POST["ocdModifiedUser"]) && isset($_POST["ocdModifiedDate"]))
    {
        $ocdOrderCancelDiscountID = $_POST["ocdOrderCancelDiscountID"];
        $ocdOrderTakingID = $_POST["ocdOrderTakingID"];
        $ocdType = $_POST["ocdType"];
        $ocdDiscountType = $_POST["ocdDiscountType"];
        $ocdDiscountAmount = $_POST["ocdDiscountAmount"];
        $ocdReason = $_POST["ocdReason"];
        $ocdModifiedUser = $_POST["ocdModifiedUser"];
        $ocdModifiedDate = $_POST["ocdModifiedDate"];
    }
    ...
    //ordertaking insert **ไม่ต้อง delete,insert แล้ว boardcast ไปตัวเอง
    {
        //query statement
    ...
    //orderCancelDiscount insert
    //query statement
    $sql = "INSERT INTO OrderCancelDiscount(OrderTakingID, Type, DiscountType, DiscountAmount, Reason, ModifiedUser, ModifiedDate) VALUES ('$otOrderTakingID', '$ocdType', '$ocdDiscountType', '$ocdDiscountAmount', '$ocdReason', '$ocdModifiedUser', '$ocdModifiedDate')";
    $ret = doQueryTask($sql);
    if($ret != "")
    {
        mysqli_rollback($con);
    ...
    //sync generated id back to app
    //select row ที่แก้ไข ขึ้นมาเก็บไว้
    $sql = "select $ocdOrderCancelDiscountID as OrderCancelDiscountID, 1 as ReplaceSelf, '$ocdModifiedUser' as ModifiedUser";
    $selectedRow = getSelectedRow($sql);
    ...
    //select row ที่แก้ไข ขึ้นมาเก็บไว้
    $ocdOrderCancelDiscountID = $newID;
    $sql = "select *, 1 IdInserted from OrderCancelDiscount where OrderCancelDiscountID = '$ocdOrderCancelDiscountID'";
    $selectedRow = getSelectedRow($sql);
    ...
    $response = array('status' => '1', 'sql' => $sql,'returnID' => $otOrderTakingID);
